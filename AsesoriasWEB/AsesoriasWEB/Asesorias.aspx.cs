...
namespace AsesoriasWEB
{
  public partial class Asesorias : System.Web.UI.Page
  {
    ...
    {
      Boolean resp = true;
      if (txHora.Text == null || txHora.Text.Equals("") ||
          txLugar.Text == null || txLugar.Text.Equals(""))
      {
      ...
      {
        lbResp.Text = "No puedes poner una fecha anterior a la de hoy.";
        resp = false;
      }
    ...
    public String altaAsesoria(String fecha, String hora, String lugar, String modalidad, String estado, int cuAsesor, int cuAsesorado, String idMateria)
    {
      ...
      if (cmd.ExecuteNonQuery() > 0)
        resp = "se mandó la petición";
      else
        resp = "no se pudo concretar la petición";
      con.Close();
      return resp;
    }
    ...
    protected void btAsesoria_Click(object sender, EventArgs e)
    {
      if (valida())
      {
        ...
        if (cuentaAsesorias(cuAsesor, cuAsesorado) < 5)
          ...
          lbResp.Text = "Solo puedes tener 5 asesorias pendientes con el mismo asesor";
      }
    }
  }
}